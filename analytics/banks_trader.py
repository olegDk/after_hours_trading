...
class BanksTrader(BaseTrader):
    def __init__(self):
        super().__init__()
        print('Initializing BanksTrader')
    ...
    def get_order(self,
                  ...
                  ask_venue,
                  std_err) -> dict:
        print(f'From BanksTrader get order if exists for '
              f'symbol: {symbol}')
        side_params = {
            # Long params
            BUY: {
        ...
        trade_flag = delta_long >= std_err/2 or delta_short <= -std_err*2  # change
        if trade_flag:

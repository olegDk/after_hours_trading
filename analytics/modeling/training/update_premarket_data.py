...
def get_data_for_tickers(tickers: list,
    ...
    if 'ticker_premarket_data' not in os.listdir(training_path):
        os.mkdir(f'{training_path}/ticker_premarket_data')

    ticker_premarket_data_path = f'{training_path}/ticker_premarket_data'

    for ticker in tickers:
        saved_data = pd.DataFrame()
        try:
            saved_data = \
                pd.read_csv(f'{ticker_premarket_data_path}/ticker_premarket_{ticker}.csv',
                            index_col=0)
        ...
        if not ticker_data.empty:
            if not saved_data.empty:
                ...
                saved_data.to_csv(f'{ticker_premarket_data_path}/ticker_premarket_{ticker}.csv')
            else:
                ticker_data = ticker_data[~ticker_data.index.duplicated(keep='first')]
                ticker_data.to_csv(f'{ticker_premarket_data_path}/ticker_premarket_{ticker}.csv')
        else:
